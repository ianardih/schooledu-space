...
export default function MitraAdmin() {
  ...
  useEffect(() => {
    fetch("/api/content/mitra").then((r) => r.json()).then(setData);
  }, []);

  const updateItem = (index: number, field: keyof MitraItem, value: string) => {
    if (!data) return;
  ...
  const addItem = () => {
    ...
    setData({
      ...data,
      items: [...data.items, { id: maxId + 1, name: "", logo: "", type: "" }],
    });
  };
  ...
  const save = async () => {
    if (!data) return;
    setSaving(true);
    const res = await fetch("/api/content/mitra", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
    });
    if (res.ok) {
      setMessage("Mitra berhasil disimpan!");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Mitra & Rekanan</h1>
          <p className="text-gray-500">Kelola mitra dan rekanan sekolah</p>
        </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">Mitra ({data.items.length})</h2>
        ...
        {data.items.map((item, index) => (
          <Card key={item.id}>
            <CardContent className="pt-6 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Mitra {index + 1}</span>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <Label>Tipe</Label>
                  <Input value={item.type} onChange={(e) => updateItem(index, "type", e.target.value)} placeholder="Industri Teknologi" />
                </div>
              </div>
            </CardContent>
          </Card>
