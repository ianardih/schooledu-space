...
const iconOptions = ["Award", "Handshake", "Briefcase", "Laptop", "TrendingUp", "GraduationCap", "Star", "Shield", "Users", "BookOpen"];
...
export default function KeunggulanAdmin() {
  ...
  useEffect(() => {
    fetch("/api/content/keunggulan").then((r) => r.json()).then(setData);
  }, []);
  ...
  const addItem = () => {
    ...
    setData({
      ...data,
      items: [...data.items, { id: maxId + 1, icon: "Award", title: "", description: "" }],
    });
  };
  ...
  const save = async () => {
    if (!data) return;
    setSaving(true);
    const res = await fetch("/api/content/keunggulan", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
    });
    if (res.ok) {
      setMessage("Keunggulan berhasil disimpan!");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Keunggulan Sekolah</h1>
          <p className="text-gray-500">Kelola keunggulan yang ditampilkan di landing page</p>
        </div>
